 ...
 */
export default class UserController implements IEntitiesController {
	...
	async createOne(ctx: Koa.Context, next: () => Promise<any>) {
		...
		const result = Joi.validate(newItem, this.validationSchemas.CreateUserSchema);
		if (result.error === null) {
			await this.collectionAdapter.create(newItem)
				.then((data: any) => {
					ctx.body = { data };
				})
				.catch((err: any) => {
	...
	async updateById(ctx: Koa.Context, next: () => Promise<any>) {
		...
		const result = Joi.validate(updateItem, this.validationSchemas.UpdateUserSchema);
