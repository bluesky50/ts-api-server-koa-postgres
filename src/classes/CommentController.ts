...
/**
 * Controller class. The controller is used by the routerBuilder function to create a router.
 */
export default class CommentController implements IEntitiesController {
	...
	async createOne(ctx: Koa.Context, next: () => Promise<any>) {
		...
		const result = Joi.validate(newItem, this.validationSchemas.CreateCommentSchema);
	...
	async updateById(ctx: Koa.Context, next: () => Promise<any>) {
		...
		const result = Joi.validate(updateItem, this.validationSchemas.UpdateCommentSchema);
